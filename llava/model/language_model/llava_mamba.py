...
from mamba_ssm.models.mixer_seq_simple import MambaConfig, MixerModel, MambaLMHeadModel # cite: 1.1, 2.1
from mamba_ssm.models.mixer_seq_simple.model import MambaForCausalLM # MambaForCausalLM is an alias for MambaLMHeadModel # cite: 1.1
# Also import RMSNorm and related functions if needed for manual forward pass
try:
    from mamba_ssm.ops.triton.layer_norm import RMSNorm, layer_norm_fn, rms_norm_fn # cite: 1.1, 3.1
except ImportError:
    RMSNorm, layer_norm_fn, rms_norm_fn = None, None, None
    print("Warning: Triton LayerNorm / RMSNorm kernels not found. Some functionality might be limited.")


class LlavaMambaConfig(MambaConfig): # Inherit from Mamba's config # cite: 2.1
    model_type = "llava_mamba"
    # Mamba models don't use rope_scaling, so explicitly set it to None.
    rope_scaling: Optional[dict] = None


class LlavaMambaModel(LlavaMetaModel, MixerModel): # LlavaMetaModel for multimodal, MixerModel for Mamba backbone # cite: 1.1
    config_class = LlavaMambaConfig

    def __init__(self, config: MambaConfig):
        super().__init__(config)
        # MixerModel's __init__ will set up self.embedding, self.layers, self.norm_f. # cite: 1.1

    def embed_tokens(self, x):
        """
        Exposes the embedding layer of MixerModel, as expected by LlavaMetaModel.
        MixerModel's embedding layer is named 'embedding'. # cite: 1.1
        """
        return self.embedding(x)


class LlavaMambaForCausalLM(MambaLMHeadModel, LlavaMetaForCausalLM): # Inherit from MambaLMHeadModel for Mamba's Causal LM, and LlavaMetaForCausalLM for multimodal capabilities # cite: 1.1
    config_class = LlavaMambaConfig
    ...
    def __init__(self, config: MambaConfig):
        ...
        # This will set up self.backbone (an instance of MixerModel) and self.lm_head. # cite: 1.1
        super().__init__(config)
        ...
        # MambaLMHeadModel stores it at self.backbone. # cite: 1.1
        self.model = self.backbone
        
        # self.lm_head is already set by MambaLMHeadModel's __init__. # cite: 1.1
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None, # Mamba models do not use attention_mask for their core computation
        position_ids: Optional[torch.LongTensor] = None, # Mamba models do not use position_ids
        past_key_values: Optional[List[torch.FloatTensor]] = None, # Mamba uses `inference_params` for state management
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = None, # Controls Mamba's internal state via `inference_params`
        output_attentions: Optional[bool] = None, # Mamba models do not produce attention weights
        output_hidden_states: Optional[bool] = None, # MixerModel can output hidden states
        ...
        input_ids, position_ids, attention_mask, past_key_values_llava_prepped, inputs_embeds, labels = \
        ...
        if use_cache and past_key_values_llava_prepped is None:
            ...
            mamba_inference_params = self.model.allocate_inference_cache(
                inputs_embeds.shape[0], inputs_embeds.shape[1], dtype=inputs_embeds.dtype
            )
        ...
        if inputs_embeds is not None:
            ...
            # Iterate through the MixerModel's layers (self.model.layers). # cite: 1.1
            for layer in self.model.layers:
                # Each layer is a `Block`, which contains the Mamba/Mamba2 mixer.
                # Block's forward takes `hidden_states`, `residual`, `inference_params`. # cite: 1.1
            ...
            # Apply final normalization, mirroring MixerModel's end processing. # cite: 1.1
            if not self.model.fused_add_norm: # MixerModel uses this flag # cite: 1.1
                hidden_states_after_norm = (current_hidden_states + residual) if residual is not None else current_hidden_states
                hidden_states_after_norm = self.model.norm_f(hidden_states_after_norm.to(dtype=self.model.norm_f.weight.dtype)) # MixerModel's final norm # cite: 1.1
            else:
                if rms_norm_fn is None:
                    raise ImportError("RMSNorm / layer_norm_fn required for fused_add_norm path and not found.")
                hidden_states_after_norm = rms_norm_fn( # Use Triton RMSNorm if fused # cite: 1.1
                    current_hidden_states,
                    self.model.norm_f.weight,
                    self.model.norm_f.bias,
                    eps=self.model.norm_f.eps,
                    residual=residual,
                    prenorm=False, # As per MixerModel's forward # cite: 1.1
                    residual_in_fp32=self.model.residual_in_fp32, # From MixerModel config # cite: 1.1
                    is_rms_norm=isinstance(self.model.norm_f, RMSNorm)
                )

            logits = self.lm_head(hidden_states_after_norm) # Apply the language model head
            ...
            if not return_dict:
                ...
                output = (logits, next_cache, hidden_states_after_norm if output_hidden_states else None, None)
            ...
            return CausalLMOutputWithPast(
                ...
                hidden_states=hidden_states_after_norm if output_hidden_states else None,
            ...
            # the base `MambaLMHeadModel`'s forward method. # cite: 1.1
            # It handles its own embedding lookup and basic state management.

            # MambaLMHeadModel's forward expects `input_ids` and `inference_params`. # cite: 1.1
            ...
            mamba_output = super().forward(
                ...
                # position_ids is ignored by MambaLMHeadModel.forward # cite: 1.1
            )
            
            logits = mamba_output.logits # MambaLMHeadModel.forward returns a namedtuple with 'logits' # cite: 1.1
            ...
            if not return_dict:
                # MambaLMHeadModel returns a namedtuple `CausalLMOutput(logits=lm_logits)`. # cite: 1.1
            ...
            return CausalLMOutputWithPast(
                loss=loss,
                logits=logits,
                past_key_values=None, # MambaLMHeadModel.forward doesn't return this directly # cite: 1.1
                hidden_states=None, # MambaLMHeadModel.forward doesn't return this directly # cite: 1.1
                attentions=None, # Mamba has no attention mechanism
            )
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None, # Corresponds to input_ids from tokenizer
        ...
        **kwargs,
        ...
        if "inputs_embeds" in kwargs:
            ...
            raise NotImplementedError("`inputs_embeds` should be handled by `prepare_inputs_for_generation` for multimodal inputs.")
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        if images is not None:
            ...
            input_ids_multimodal, position_ids_multimodal, attention_mask_multimodal, \
            past_key_values_multimodal, inputs_embeds_multimodal, labels_multimodal = \
                self.prepare_inputs_labels_for_multimodal(
            ...
            model_inputs = {
                ...
                "use_cache": kwargs.get("use_cache", False), # Ensure use_cache is passed correctly
